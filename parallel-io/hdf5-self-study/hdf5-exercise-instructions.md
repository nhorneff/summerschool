...
- Roihu:
    1. `module load hdf5`
    2. Compile with `mpicxx my_file.cpp -lhdf5`, or `mpif90 my_file.F90 -I/appl/soft/spack/core/v2026_03/x86_64/gcc152_ec/install_dir/zen5/gcc-15.2.0/hdf5-1.14.6-2d66cw/include -lhdf5_fortran` for Fortran code.
...
On many Linux distributions you can obtain HDF5 development libraries through the system package manager. E.g. on Ubuntu you can use
```
apt install libhdf5-openmpi-dev hdf5-tools
```
`hdf5-tools` installs `h5dump`, `h5ls` which are needed for the exercises.

Note that `apt` also has package `libhdf5-dev` which provides `libhdf5-serial-dev`, i.e. a serial non-MPI library. The parallel HDF5 exercises will not work with a serial implementation.
