...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        MPI_Send(message.data(), msgsize, MPI_INT, 1, message_tag, MPI_COMM_WORLD);
        MPI_Recv(receiveBuffer.data(), msgsize, MPI_INT, 1, message_tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        
        printf("Rank %i received %i elements, first %i\n", rank, msgsize, receiveBuffer[0]);
    }
    else if (rank == 1) {

        MPI_Recv(receiveBuffer.data(), msgsize, MPI_INT, 0, message_tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Send(message.data(), msgsize, MPI_INT, 0, message_tag, MPI_COMM_WORLD);
